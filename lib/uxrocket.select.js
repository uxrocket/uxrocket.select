...
(function($) {
    ...
    var Select = function(el, options, selector) {
        ...
        $el.on(events.keydown, function(e) {
            $('.' + ns.selected).removeClass(ns.selected).removeClass(opts.selected);
        });
    ...
    var setLayout = function($el) {
        ...
        if($el.parent().is('.' + ns.rocketWrap)) {
            $el.parent().addClass(ns.wrap + columns).addClass(opts.wrapper);
        }
        else if($el.parent().is('label') && $el.is(':only-child')) {
            $el.parent().addClass(ns.rocketWrap + ' ' + ns.wrap + columns).addClass(opts.wrapper);
        }
        else if(!$el.parent().is('label')) {
            $el.wrap('<label class="' + ns.rocketWrap + ' ' + ns.wrap + ' ' + opts.wrapper + columns + '"></label>');
        }
        else {
            $el.wrap('<span class="' + ns.rocketWrap + ' ' + ns.wrap + ' ' + opts.wrapper + columns + '"></span>');
        }

        $el.after(
            '<span class="' + ns.current + ' ' + opts.current + '">' +
            '<span class="' + ns.text + '">' + $current + '</span>' +
            '<span class="' + ns.arrow + ' ' + opts.arrow + '"></span>' +
            '</span>');
    ...
    var toggleList = function($el) {
        var data = $el.attr('id'),
            ...
            $list = $('.' + ns.list).filter('[data-select="' + data + '"]');

        $('.' + ns.list).not($list).hide();
    ...
    var selectOption = function($option) {
        ...
        $list.find('.' + ns.selected).removeClass(ns.selected).removeClass(_opts.selected);
    ...
    var builtOptions = function($el) {
        ...
        list = '<ul id="' + ns.list + '-' + _opts.index + '" data-select="' + id + '" class="' + ns.list + ' ' + opts.list + readonly + '">';

        $el.find('option').each(function() {
            var $o = $(this);

            list += '<li class="group ' + ns.item + '"><a class="group ' + ns.option + ' ' + opts.option;

            if($o.is(':selected')) {
                list += ' ' + ns.selected + ' ' + opts.selected;
            }
