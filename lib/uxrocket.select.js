...
;(function($){
    ...
    var Select = function(el, options){
        ...
        // set options
        opts = $.extend({}, defaults, options, $el.data(), {'id': id, 'index': index});
    ...
    var bindUIActions = function($el){
        var _opts = $el.data('opts');

        $('#uxitd-select-list-' + _opts.index).on('click', '.uxitd-select-option', function(e){
            e.preventDefault();
            selectOption($(this));
        });
    ...
    var builtOptions = function($el){
        ...
        list = '<ul id="uxitd-select-list-' + _opts.index + '" data-select="' + id + '" class="uxitd-select-list ' + opts.list + '">';
