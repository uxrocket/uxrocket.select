...
(function(factory) {
    ...
    Select.prototype.select = function($selected) {
        ...
        /*
        if ( this.multiple && this.$el.data('select-all') && $selected.parent().hasClass('uxr-select-selectAll') ) {
    ...
    Select.prototype.deselectAll = function (list, instance) {
        ...
        list.each( function(i) {
            ...
            var id = $(this).attr('id');
            $('#' + id).removeClass( utils.getClassname('selected') );
            if ( subItems.length ) {
        ...
        list.each(function(index, el) {
            ...
            var val = $(el).find('> a').data('value');
            var options = _this.$el.find('option');
    ...
    Select.prototype.selectAll = function (list, instance) {
        ...
        list.each( function() {
            var subItems = $(this).find('li');
            var id = $(this).attr('id');
            $('#' + id).addClass( utils.getClassname('selected') );
            if ( subItems.length ) {
    ...
    Select.prototype.setMultipleInfoMessage = function () {
        var activeList = $('.uxr-select-list').find('li');
        var selectionText = utils.getClassname('selectionText');
        var totalList = activeList.filter(function( index ) {
            return ( !$( this ).hasClass( 'uxr-select-group' ) );
        });
        var selectedList = activeList.filter(function( index ) {
            return $( this ).hasClass( utils.getClassname('selected') );
        });
        var selectedLength = ( (selectedList.length - 1) < 0 ) ? 0 : (selectedList.length - 1);
        this.$selection.find('.' + selectionText).html(this.options.multipleInfoMessage + ' ' + selectedLength + '/' + (totalList.length - 1) );
    };
  
    Select.prototype.prepareToggleAll = function ($selected) {
        var instance = $(utils.escapeSelector('#' + this.$drop.data('select'))).data(ns.data);
        var list = this.$activeSearchList ? this.$activeSearchList.find('li') : this.$list.find('li');
        if ( this.selectAllValue ) {
            $selected.parent().addClass( utils.getClassname('selected') );
            $selected.text( this.options.text.deSelectAll );
            this.selectAll(list, instance);
            this.selectAllValue = false;
        } else {
            $selected.parent().removeClass( utils.getClassname('selected') );
            $selected.text( this.options.text.selectAll );
            this.deselectAll(list, instance);
            this.selectAllValue = true;
        }
        this.setMultipleInfoMessage();
    };
    ...
    Select.prototype.search = function(term) {
        ...
        this.$activeSearchList = $(list);
        $list.replaceWith(list);
        console.log('this.$activeSearchList', this.$activeSearchList);
    ...
    Select.prototype.prepareDrop = function() {
        ...
        this.$selectAllElement = this.$drop.find('.uxr-select-selectAll');
  ...
  */
    ...
    Select.prototype.open = function() {
        ...
        this.setDropPosition(true);
        this.$activeSearchList = null;
        // this.selectedStateBulkUpdate();
    };
  
  
    Select.prototype.selectedStateBulkUpdate = function () {
        if (!this.$list) {return;}
        var _this = this;
        ...
        if (values.length > 1) {
            ...
            values.forEach( function (value) {
                var el = _this.$list.find('[data-value=' + value + ']');
                el.parent().addClass( utils.getClassname('selected') );
            });
        } else if(values) {
            var el = _this.$list.find('[data-value=' + values + ']');
            el.parent().addClass( utils.getClassname('selected') );
        }
