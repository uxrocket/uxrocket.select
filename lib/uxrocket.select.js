...
(function(factory) {
    ...
    var ux, // local shorthand
        ...
        templates       = {
            ...
            list:      '<ul class="{{listClass}}">' +
                       '   {{#each options}}' +
                       '   <li id="{{optionClass}}-{{options.index}}"{{#if options.selected}} class="{{selectedClass}}"{{/if}}>' +
        ...
        _this.$drop
            ...
            .on(events.keyup + ' ' + events.input, '.' + _this.utils.getClassname('search') + ' input', function(e) {
                ...
                if(e.keyCode === keys.up || e.keyCode === keys.down) {
                    return _this.navigateWithArrow(keys.codes[e.keyCode]);
                }

                if(e.keyCode === keys.return) {
                    return _this.navigateWithEnter();
                }
    ...
    Select.prototype.navigateWithArrow = function(updown) {
        ...
        if(!highlighted.length) {
            selected = this.$list.find('.' + this.utils.getClassname('option')).parent().first().addClass(highlight);
        }

        else {
            selected = highlighted.removeClass(highlight)[direction]().addClass(highlight);

            console.log(listPos + ' - ' + scrollTop + ' - ' + selected.offset().top);

            if(updown === 'down') {
                if(selected.offset().top > (listPos + height - selected.height())) {
                    this.$list.scrollTop(scrollTop + selected.height());
                }
            }

            else {
                if(scrollTop > (selected.offset().top + selected.height())) {
                    this.$list.scrollTop(scrollTop - selected.height());
                }
            }
        }
